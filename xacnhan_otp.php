    ...
    if ($action === 'resend') {
        ...
        if (strlen($entered) !== 6 || !ctype_digit($entered)) {
            ...
            if (time() > $expires_at) {
                ...
                // ============================================================
                // Nếu là OTP đăng nhập (thiết bị lạ) + user chọn "Nhớ thiết bị"
                // → lưu token vào trusted_devices + set cookie 30 ngày
                // ============================================================
                if ($otp_type === 'login' && !empty($_POST['remember_device'])) {
                    $token   = bin2hex(random_bytes(32));
                    $expires = date('Y-m-d H:i:s', time() + 30 * 24 * 3600);

                    $stmt_td = $conn->prepare(
                        "INSERT INTO trusted_devices (user_id, token, expires_at) VALUES (?, ?, ?)"
                    );
                    $stmt_td->bind_param("iss", $user_id, $token, $expires);
                    $stmt_td->execute();

                    setcookie(
                        'trusted_device_' . $user_id,
                        $token,
                        time() + 30 * 24 * 3600,
                        '/',
                        '',
                        false, // đổi thành true nếu dùng HTTPS
                        true   // httpOnly
                    );
                }
                ...
                header("Location: " . ($role === 'teacher' ? 'giaovien/index.php' : 'hocsinh/index.php'));
                exit;
            }
        }
    }
...
<div class="card">
    ...
    <form method="POST" id="otpForm">
        ...
        <?php if ($otp_type === 'login'): ?>
        <label class="remember-device">
            <input type="checkbox" name="remember_device" value="1">
            <span>
                Nhớ thiết bị này trong 30 ngày
                <small>Không hỏi OTP ở thiết bị này trong 30 ngày tới</small>
            </span>
        </label>
        <?php endif; ?>
