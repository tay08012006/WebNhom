...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ma_lop'])) {
    ...
    if (empty($ma_lop)) {
        ...
        if ($class_info) {
            ...
            $sql_students = "SELECT u.id, u.hoten, u.email, u.avatar
            ...
            <tbody>
                <?php if ($total_students > 0): ?>
                    <?php $stt = 1; while ($row = $students_result->fetch_assoc()): ?>
                        <?php
                            $av_file = $row['avatar'] ?? '';
                            $av_src = !empty($av_file)
                                ? '../uploads/' . htmlspecialchars($av_file)
                                : '';
                            $av_chu = mb_strtoupper(mb_substr($row['hoten'], 0, 1, 'UTF-8'), 'UTF-8');
                            $av_colors = ['#0288d1','#26a69a','#ef5350','#ab47bc','#ff7043'];
                            $av_color = $av_colors[crc32($row['hoten']) % count($av_colors)];
                        ?>
                        <tr>
                            <td><?= $stt++ ?></td>
                            <td>
                                <div style="width:40px;height:40px;border-radius:50%;overflow:hidden;display:flex;align-items:center;justify-content:center;background:<?= $av_color ?>;color:white;font-weight:700;font-size:16px;">
                                    <?php if (!empty($av_src)): ?>
                                        <img src="<?= $av_src ?>" style="width:100%;height:100%;object-fit:cover;" 
                                             onerror="this.style.display='none';this.parentElement.textContent='<?= $av_chu ?>';">
                                    <?php else: ?>
                                        <?= $av_chu ?>
                                    <?php endif; ?>
                                </div>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="4" class="no-data">Chưa có học sinh nào khác trong lớp này.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
