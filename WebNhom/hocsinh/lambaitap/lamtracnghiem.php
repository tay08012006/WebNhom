...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .cheat-icon { font-size: 56px; margin-bottom: 12px; }
        .cheat-modal h2 {
            font-size: 22px; font-weight: 900; color: #1e293b; margin-bottom: 10px;
        }
        .cheat-modal p { font-size: 15px; color: #475569; line-height: 1.7; margin-bottom: 20px; }
        .cheat-modal .warn-count {
        ...
        .cheat-modal .warn-count span { font-size: 22px; color: #d97706; }
        .btn-resume {
            background: linear-gradient(135deg, #3b82f6, #6366f1);
        ...
        .cheat-locked-overlay.active { display: flex; }
        .locked-modal {
            background: white; border-radius: 20px;
        ...
        .locked-modal h2 { font-size: 24px; font-weight: 900; color: #dc2626; margin-bottom: 12px; }
        .locked-modal p  { font-size: 15px; color: #475569; line-height: 1.7; }
        .locked-badge {
            display: inline-block;
        ...
        /* Thanh cảnh báo trên đầu */
        .cheat-warning-bar {
            display: none;
            background: linear-gradient(90deg, #f59e0b, #ef4444);
            color: white; text-align: center;
            padding: 8px 16px; font-size: 13px; font-weight: 800;
            letter-spacing: 0.3px; position: relative; z-index: 100;
        }
        .cheat-warning-bar.show { display: block; }
...
<!-- ===== THANH CẢNH BÁO GIAN LẶN ===== -->
<div class="cheat-warning-bar" id="cheatWarningBar">
    ⚠️ Cảnh báo: Bạn đã thoát khỏi trang thi! Còn lại <span id="warnLeft">3</span> lần trước khi bị khóa.
</div>

<!-- ===== MODAL CẢNH BÁO (lần 1 & 2) ===== -->
<div class="cheat-overlay" id="cheatWarnOverlay">
    <div class="cheat-modal">
        <div class="cheat-icon">⚠️</div>
        <h2>Bạn vừa rời khỏi trang thi!</h2>
        <p>Hệ thống ghi nhận bạn đã thoát sang tab/cửa sổ khác.<br>
           Hành vi này được theo dõi và báo cáo cho giáo viên.</p>
        <div class="warn-count">
            Số lần vi phạm: <span id="violationCount">1</span> / 3
        </div>
        <br>
        <button class="btn-resume" onclick="resumeQuiz()">✅ Quay lại làm bài</button>
    </div>
</div>

<!-- ===== MODAL KHÓA BÀI (lần 3) ===== -->
<div class="cheat-locked-overlay" id="cheatLockedOverlay">
    <div class="locked-modal">
        <div class="cheat-icon">🔒</div>
        <h2>Bài thi đã bị khóa!</h2>
        <div class="locked-badge">❌ Vi phạm 3/3 lần — Bị khóa tự động</div>
        <p>Bạn đã thoát khỏi trang thi <strong>3 lần</strong>. Hệ thống đã tự động
           <strong>khóa bài thi</strong> và <strong>thông báo cho giáo viên</strong>.
        </p>
        <p style="margin-top:14px; color:#dc2626; font-weight:700;">
            Giáo viên đã được thông báo về hành vi gian lận này.
        </p>
        <div style="margin-top:24px;">
            <a href="danhsach_baithi.php?id_lop=<?= $id_lop ?>"
               style="display:inline-block; background:#64748b; color:white; padding:13px 32px;
...
// ============================================================
// CHỐNG GIAN LẶN — PHÁT HIỆN THOÁT TAB
// ============================================================
const CHEAT_KEY      = 'quiz_cheat_' + <?= json_encode($quiz_id) ?> + '_' + <?= json_encode($student_id) ?>;
const MAX_VIOLATIONS = 3;
let   cheatCount     = 0;
let   quizIsLocked   = false;
let   lastBlurTime   = 0;
const DEBOUNCE_MS    = 1500; // tránh đếm hai lần trong cùng 1 lần thoát tab

// Khởi tạo — đọc từ localStorage (phòng trường hợp reload)
function initCheatTracker() {
    const stored = localStorage.getItem(CHEAT_KEY);
    if (stored) {
        const data = JSON.parse(stored);
        cheatCount   = data.count   || 0;
        quizIsLocked = data.locked  || false;
    }

    if (quizIsLocked) {
        showLockedOverlay(); // Nếu đã bị khóa trước đó → khóa lại ngay
    } else if (cheatCount > 0) {
        updateWarningBar();  // Hiện thanh cảnh báo nếu đã vi phạm rồi
    }
}

function saveCheatState() {
    localStorage.setItem(CHEAT_KEY, JSON.stringify({ count: cheatCount, locked: quizIsLocked }));
}

// ── Phát hiện sự kiện ──────────────────────────────────────────────────────
// Dùng 1 timestamp chung cho cả 2 sự kiện để tránh đếm trùng.
// Khi thoát tab: blur bắn trước (hidden=false), rồi visibilitychange bắn (hidden=true).
// Cả hai đều gọi triggerViolation(), nhưng chỉ lần đầu trong 1.5s mới đếm.

function triggerViolation() {
    if (quizIsLocked) return;
    const now = Date.now();
    if (now - lastBlurTime < DEBOUNCE_MS) return; // đã đếm rồi, bỏ qua
    lastBlurTime = now;
    handleViolation();
}
...
function handleViolation() {
    ...
    cheatCount++;
    saveCheatState();
    reportViolationToServer(cheatCount);
...
function showWarnOverlay() {
    document.getElementById('violationCount').textContent = cheatCount;
    document.getElementById('cheatWarnOverlay').classList.add('active');
}

function resumeQuiz() {
    document.getElementById('cheatWarnOverlay').classList.remove('active');
}

function showLockedOverlay() {
    // Ẩn mọi overlay khác
    document.getElementById('cheatWarnOverlay').classList.remove('active');
    document.getElementById('cheatLockedOverlay').classList.add('active');
    // Vô hiệu hóa nút nộp bài để tránh nộp bình thường
    const btn = document.getElementById('submitBtn');
    if (btn) { btn.disabled = true; btn.textContent = '🔒 Bài đã bị khóa'; }
}

function updateWarningBar() {
    const bar  = document.getElementById('cheatWarningBar');
    const left = document.getElementById('warnLeft');
    if (!bar) return;
    const remaining = MAX_VIOLATIONS - cheatCount;
    if (remaining <= 0) {
        bar.style.background = '#dc2626';
        bar.textContent = '🔒 Bài thi đã bị khóa do vi phạm quá số lần cho phép.';
    } else {
...
// ── Báo cáo lên server qua AJAX ───────────────────────────────────────────
function reportViolationToServer(count) {
    const fd = new FormData();
    fd.append('quiz_id',        <?= json_encode($quiz_id) ?>);
    fd.append('so_lan_vi_pham', count);

    fetch('bao_cao_gianlam.php', { method: 'POST', body: fd })
        .then(r => r.json())
        .then(data => {
            if (data.status === 'locked' && !quizIsLocked) {
                quizIsLocked = true;
                saveCheatState();
                showLockedOverlay();
            }
        })
        .catch(() => {}); // Mạng lỗi → vẫn xử lý phía client
}
