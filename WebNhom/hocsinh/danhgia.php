...
// SỬA TẠI ĐÂY: Nhận cả tham số 'id' hoặc 'id_lop_hoc' để đồng bộ với nút bấm bên index.php
$id_lop_hoc = 0;
...
// Kiểm tra lớp học này có tồn tại không
...
// Xử lý khi học sinh bấm nút gửi đánh giá
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($sao_kien_thuc >= 1 && $sao_kien_thuc <= 5 && $sao_su_pham >= 1 && $sao_su_pham <= 5 && $sao_ho_tro >= 1 && $sao_ho_tro <= 5) {
        // Lưu dữ liệu hoàn toàn ẩn danh (không lưu user_id của học sinh)
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Nunito', sans-serif; }
        ...
        .wrapper { background: white; max-width: 550px; width: 100%; padding: 35px; border-radius: 20px; box-shadow: 0 10px 25px rgba(0,0,0,0.05); border: 1px solid #e2e8f0; }
        ...
        .alert-error { background: #fef2f2; color: #b91c1c; padding: 12px; border-radius: 8px; font-weight: 700; font-size: 14px; margin-bottom: 20px; border: 1px solid #fca5a5; }
        ...
        .cham-sao label { cursor: pointer; width: 36px; height: 36px; margin-right: 6px; background-image: url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='%23cbd5e1'><path d='M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z'/></svg>"); background-repeat: no-repeat; background-position: center; background-size: contain; transition: transform 0.1s ease; }
        .cham-sao label:hover { transform: scale(1.15); }
        .cham-sao input:checked ~ label, .cham-sao label:hover, .cham-sao label:hover ~ label { background-image: url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='%23ea580c'><path d='M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z'/></svg>"); }
        
        textarea { width: 100%; height: 110px; border-radius: 10px; border: 1px solid #cbd5e1; padding: 12px; resize: none; box-sizing: border-box; outline: none; font-size: 14px; transition: border-color 0.2s; }
        textarea:focus { border-color: #ea580c; box-shadow: 0 0 0 3px rgba(234,88,12,0.1); }
        .actions { display: flex; gap: 15px; margin-top: 25px; }
        .btn-back { display: block; text-align: center; width: 35%; padding: 12px; background: #f1f5f9; color: #475569; text-decoration: none; border-radius: 10px; font-weight: 700; font-size: 15px; }
        .btn-back:hover { background: #e2e8f0; }
        button { width: 65%; padding: 12px; background: #ea580c; color: white; border: none; border-radius: 10px; font-size: 15px; cursor: pointer; font-weight: 700; }
