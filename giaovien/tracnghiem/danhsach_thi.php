...
// MỤC 1: KHỞI TẠO SESSION & KIỂM TRA QUYỀN TRUY CẬP CỦA GIÁO VIÊN

ini_set('session.name', 'GV_SESSION');
session_start();
require_once '../../config.php';

// Bảo mật: Chỉ cho phép tài khoản đăng nhập với vai trò là 'teacher' (Giáo viên) truy cập dữ liệu
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'teacher') {
    die("Bạn không có quyền truy cập.");
}

// MỤC 2: TIẾP NHẬN VÀ KIỂM TRA TÍNH HỢP LỆ CỦA THAM SỐ ĐẦU VÀO (GET REQUEST)
...
// MỤC 3: TRUY VẤN THÔNG TIN CHI TIẾT BÀI TRẮC NGHIỆM VÀ LỚP HỌC TƯƠNG ỨNG
...
// MỤC 4: XỬ LÝ BỘ LỌC ĐIỂM SỐ VÀ TIÊU CHÍ SẮP XẾP DANH SÁCH KẾT QUẢ THI
...
// Cấu hình câu lệnh SQL cho việc sắp xếp danh sách kết quả thi theo yêu cầu của Giáo viên
$order_sql = "u.hoten ASC"; // Mặc định: Tên học sinh tăng dần từ A đến Z
...
// Phân loại và xây dựng điều kiện lọc khoảng điểm (Giỏi, Khá, Trung bình, Yếu, Đạt, Trượt)
$where_diem = "";
$params     = [$quiz_id];
$types      = "i";
...
// Thực thi câu lệnh truy vấn lấy danh sách học sinh đã làm bài dựa trên bộ lọc đã chọn
...
// MỤC 5: TRUY VẤN DỮ LIỆU THỐNG KÊ TỔNG QUAN (TỔNG SỐ BÀI, ĐIỂM TB, CAO/THẤP NHẤT)
...
// MỤC 6: KHỞI TẠO VÀ TRUY VẤN NHẬT KÝ HỌC SINH VI PHẠM QUY CHẾ (THOÁT TAB/GIAN LẬN)
// Tự động kiểm tra và khởi tạo bảng lưu trữ lịch sử vi phạm nếu cơ sở dữ liệu chưa có
$conn->query(
...
// Lấy thông tin chi tiết danh sách các thí sinh có hành vi rời màn hình/thoát tab khi làm bài
$cheat_list = [];
$cheat_map  = [];
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* THANH ĐIỀU HƯỚNG - TIÊU ĐỀ TRANG */
        .page-header {
        ...
        /* THẺ THỐNG KÊ SỐ LIỆU TỔNG QUAN */
        .stat-grid {
        ...
        /* THANH BỘ LỌC TÌM KIẾM VÀ SẮP XẾP */
        .filter-bar {
        ...
        /* BẢNG DỮ LIỆU HIỂN THỊ HỌC SINH */
        ...
        /* THÀNH PHẦN THÔNG TIN CHI TIẾT TRONG BẢNG (AVATAR, ĐIỂM, GIỚI TÍNH) */
        ...
        /* KHU VỰC NHẬP NHẬN XÉT CỦA GIÁO VIÊN */
        ...
        /* KHU VỰC HIỂN THỊ CẢNH BÁO VI PHẠM (CHEATING STYLES) */
        .cheat-section {
        ...
        .cheat-section-header h3 { color: #c53030; font-size: 15px; font-weight: 900; display: flex; align-items: center; gap: 10px; }
        .cheat-count-pill {
            background: white; color: #e53e3e; border: 1.5px solid #fca5a5;
            border-radius: 20px; padding: 3px 14px; font-size: 13px; font-weight: 900;
        }
        .cheat-table { width: 100%; border-collapse: collapse; }
        .cheat-table th {
            background: #fff5f5; color: #c53030; font-size: 12px; font-weight: 800;
            text-transform: uppercase; letter-spacing: 0.4px; padding: 10px 16px; text-align: left;
            border-bottom: 1px solid #fecaca;
        }
        .cheat-table td { padding: 13px 16px; border-bottom: 1px solid #fff1f2; font-size: 14px; vertical-align: middle; }
        ...
        .badge-lock {
            display: inline-flex; align-items: center; gap: 5px; background: #fff0f0; color: #c53030;
            border: 1.5px solid #fca5a5; border-radius: 8px; padding: 4px 12px; font-size: 12px; font-weight: 800;
        }
        .badge-warn {
            display: inline-flex; align-items: center; gap: 5px; background: #fef9ec; color: #b45309;
            border: 1.5px solid #fcd34d; border-radius: 8px; padding: 4px 12px; font-size: 12px; font-weight: 800;
        }
        .cheat-progress { display: flex; gap: 3px; align-items: center; }
        .cheat-dot { width: 10px; height: 10px; border-radius: 50%; }
        .cheat-dot.filled { background: #f87171; }
        .cheat-dot.empty  { background: #fecaca; }
        
        .cheat-badge {
            display: inline-flex; align-items: center; gap: 5px; background: #fee2e2; color: #991b1b;
            border: 1.5px solid #fca5a5; border-radius: 8px; padding: 3px 10px; font-size: 12px; font-weight: 800;
        }
        .cheat-warn-badge {
            display: inline-flex; align-items: center; gap: 5px; background: #fef3c7; color: #92400e;
            border: 1.5px solid #fcd34d; border-radius: 8px; padding: 3px 10px; font-size: 12px; font-weight: 800;
        }
    </style>
</head>
<body>
...
<div class="container">
    ...
    <form method="GET" action="" class="filter-bar">
        ...
        <a href="?quiz_id=<?= $quiz_id ?>&malop=<?= urlencode($ma_lop) ?>" class="reset-btn">↺ Đặt lại</a>
    ...
    <div class="cheat-section">
        <div class="cheat-section-header">
            <h3>🚨 Danh sách học sinh vi phạm (Rời khỏi màn hình bài thi)</h3>
        ...
        <table class="cheat-table">
            <thead>
                <tr>
                    ...
                    <th style="text-align:center">Trạng thái bài làm</th>
    ...
    <?php else: ?>
    <div style="background:white;border-radius:14px;padding:14px 20px;margin-bottom:20px; border:1.5px solid #dcfce7;display:flex;align-items:center;gap:10px;">
        <span style="font-size:20px">✅</span>
        <span style="font-size:14px;font-weight:700;color:#15803d">Hệ thống không phát hiện trường hợp học sinh vi phạm nào trong bài thi này.</span>
    </div>
    <?php endif; ?>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th>Thời gian nộp</th>
                    <th style="text-align:center">Giám sát vi phạm</th>
            ...
            <?php $stt = 1; while ($row = $results->fetch_assoc()):
                ...
                // Định dạng hiển thị màu sắc theo từng phổ điểm của học sinh
                $score = floatval($row['score']);
                $score_class = $score >= 8 ? 'score-gioi' : ($score >= 6.5 ? 'score-kha' : ($score >= 5 ? 'score-tb' : 'score-yeu'));

                // Tính toán tỷ lệ phần trăm số câu trả lời đúng
                $pct = $row['total_questions'] > 0 ? round($row['correct_count'] / $row['total_questions'] * 100) : 0;
            ?>
                <tr>
                    <td class="stt"><?= $stt++ ?></td>
                    ...
                    <td class="correct-cell">
                        <div class="correct-frac"><?= $row['correct_count'] ?> / <?= $row['total_questions'] ?></div>
                        <div class="correct-pct"><?= $pct ?>%</div>
                    </td>

                    <td class="score-cell">
                        <div class="score-big <?= $score_class ?>"><?= number_format($score, 1) ?></div>
                    ...
                    <td class="comment-cell">
                        <div class="comment-form">
                            ...
                            <button class="comment-save-btn" onclick="saveComment(this, <?= $row['id'] ?>)">💾 Lưu</button>
                        </div>
...
async function saveComment(btn, resultId) {
    ...
    // Khóa trạng thái nút bấm tạm thời để tránh việc nhấn liên tục (Double Request)
    btn.disabled = true;
    const oldText = btn.innerHTML;
    btn.innerHTML = '⏳ Lưu...';
    try {
        // Gửi dữ liệu nhận xét qua cổng API backend 'luu_nhanxet.php'
        ...
        // Phản hồi UI động dựa trên kết quả trả về từ máy chủ
        if (data.success) {
        ...
        // Trả lại trạng thái hoạt động bình thường cho nút bấm
        btn.disabled = false;
