...
<?php
// ── Lấy cảnh báo gian lận chưa đọc của lớp này ─────────────────────────
$conn->query(
...
$stmt_alerts = $conn->prepare(
    "SELECT tb.noi_dung, tb.ngay_tao, tb.id AS tb_id,
            cl.so_lan_vi_pham, cl.student_id AS hs_id,
            u.hoten AS ten_hs,
            q.title AS ten_baithi,
            cl.quiz_id
     FROM thong_bao tb
     JOIN quiz_cheating_log cl
          ON tb.noi_dung LIKE CONCAT('%qid=', cl.quiz_id, ';sid=', cl.student_id, '%')
     JOIN quizzes q  ON q.id  = cl.quiz_id
     JOIN users  u   ON u.id  = cl.student_id
     WHERE tb.class_id    = ?
       AND tb.nguoi_gui_id = 0
       AND tb.noi_dung    LIKE '%GIAN LẬN%'
     ORDER BY tb.ngay_tao DESC
     LIMIT 20"
);
$stmt_alerts->bind_param("i", $class_id);
$stmt_alerts->execute();
...
<div class="cheat-alert-box">
    <div class="cheat-alert-header">
        <span class="cheat-alert-icon">🚨</span>
        <div>
            <strong>Phát hiện <?= $alert_count ?> trường hợp gian lận!</strong>
            <span>Các học sinh dưới đây đã bị khóa bài thi do thoát tab quá 3 lần.</span>
        </div>
    </div>
    <div class="cheat-alert-list">
        <?php while ($al = $alerts->fetch_assoc()): ?>
        <div class="cheat-alert-row">
            <div class="cheat-hs-info">
                <span class="cheat-hs-name">👤 <?= htmlspecialchars($al['ten_hs']) ?></span>
                <span class="cheat-quiz-name">📝 <?= htmlspecialchars($al['ten_baithi']) ?></span>
            </div>
            <div class="cheat-right">
                <span class="cheat-count-badge">🔒 <?= $al['so_lan_vi_pham'] ?>/3 lần</span>
                <span class="cheat-time"><?= date('H:i d/m/Y', strtotime($al['ngay_tao'])) ?></span>
                <a href="danhsach_thi.php?quiz_id=<?= $al['quiz_id'] ?>&malop=<?= urlencode($ma_lop) ?>"
                   class="cheat-view-btn">Xem bài →</a>
            </div>
        </div>
        <?php endwhile; ?>
    </div>
...
.cheat-alert-icon { font-size: 28px; line-height: 1; flex-shrink: 0; }
.cheat-alert-header strong {
    display: block; font-size: 16px; font-weight: 900; color: #c53030; margin-bottom: 3px;
}
.cheat-alert-header span { font-size: 13px; color: #742a2a; }
.cheat-alert-list { display: flex; flex-direction: column; gap: 10px; }
.cheat-alert-row {
    display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px;
    background: white; border: 1.5px solid #fed7d7;
    border-radius: 10px; padding: 12px 16px;
}
.cheat-hs-info { display: flex; flex-direction: column; gap: 3px; }
.cheat-hs-name { font-size: 15px; font-weight: 800; color: #1a202c; }
.cheat-quiz-name { font-size: 13px; color: #718096; }
.cheat-right { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; }
.cheat-count-badge {
    background: #fee2e2; color: #991b1b;
    border: 1.5px solid #fca5a5; border-radius: 8px;
    padding: 4px 12px; font-size: 13px; font-weight: 800;
}
.cheat-time { font-size: 12px; color: #a0aec0; }
.cheat-view-btn {
...
.cheat-view-btn:hover { background: #c53030; }
</style>
<?php endif; ?>
