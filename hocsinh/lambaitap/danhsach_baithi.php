...
$stmt_quiz = $conn->prepare("
    SELECT q.*,
        ...
        (SELECT nhan_xet_gv FROM quiz_results WHERE quiz_id = q.id AND student_id = ?) AS nhan_xet_gv
    FROM quizzes q
    WHERE q.class_id = ?
    ORDER BY q.created_at DESC
");
$stmt_quiz->bind_param("iiiiii", $student_id, $student_id, $student_id, $student_id, $student_id, $id_lop);
$stmt_quiz->execute();
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .badge-comment {
            ...
            animation: pulseComment 2s infinite;
        }
        @keyframes pulseComment {
            0%, 100% { box-shadow: 0 0 0 0 rgba(29,78,216,0.3); }
            50%       { box-shadow: 0 0 0 5px rgba(29,78,216,0); }
        }

        /* PREVIEW NHẬN XÉT TRONG CARD */
        .comment-preview {
            margin-top: 10px; padding: 10px 14px;
            background: #eff6ff; border-left: 3px solid #3b82f6;
            border-radius: 0 8px 8px 0; font-size: 13px; color: #1e3a8a;
...
<div class="container">
    ...
    <?php if ($quizzes->num_rows > 0): ?>
        ...
        <?php while ($quiz = $quizzes->fetch_assoc()):
            ...
            $nhan_xet = trim($quiz['nhan_xet_gv'] ?? '');
        ...
        ?>
            <div class="quiz-card <?= $da_lam ? 'done' : 'pending' ?>" <?= ($da_lam && $nhan_xet) ? 'style="border-left-color:#3b82f6"' : '' ?>>
                <div class="quiz-info">
                    ...
                    <?php if ($da_lam && $nhan_xet !== ''): ?>
                    <div class="comment-preview">
                        <div class="comment-label">📝 Nhận xét của giáo viên</div>
                        <div class="comment-text"><?= htmlspecialchars($nhan_xet) ?></div>
                    </div>
                    <?php endif; ?>
                </div>
                ...
                <div class="quiz-right">
                    <?php if ($da_lam): ?>
                        ...
                        <?php if ($nhan_xet !== ''): ?>
                        <span class="badge badge-comment">💬 Có nhận xét mới</span>
                        <?php endif; ?>
