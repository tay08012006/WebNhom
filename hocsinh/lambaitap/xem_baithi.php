...
// Đẩy về trang đăng nhập nếu tài khoản không phải là học sinh
if (!isset($_SESSION['role']) || $_SESSION['role'] != 'student') {
    header("Location: ../trangdangnhap.php");
    exit();
}

// Lấy ID bài thi và ID lớp từ URL
$quiz_id = isset($_GET['quiz_id']) ? intval($_GET['quiz_id']) : 0;
$id_lop  = isset($_GET['id_lop']) ? intval($_GET['id_lop']) : 0;

// Báo lỗi nếu mã bài thi không hợp lệ
if ($quiz_id <= 0) die("Mã bài thi không hợp lệ.");

// Lấy thông tin tiêu đề bài thi từ cơ sở dữ liệu
...
// Lấy điểm tổng quát và kết quả của học sinh
...
// Lấy chi tiết từng câu trả lời để đối chiếu với đáp án đúng
...
<head>
    ...
    <style>
        /* Cài đặt chung để loại bỏ khoảng cách mặc định */
        * { 
        ...
        /* Cài đặt font chữ và màu nền cho trang */
        body { 
            font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; 
            background: #f0f4f8; 
        }
        
        /* Banner xanh trên cùng chứa tên bài thi */
        .header { 
        ...
        /* Thanh trắng hiển thị tổng quan điểm số */
        .score-bar { 
            background: white; 
            padding: 20px 32px; 
            display: flex; 
        ...
        /* Tiêu đề của từng mục điểm (Điểm, Câu đúng...) */
        .score-item .label { 
            font-size: 12px; 
            color: #718096; 
            font-weight: 700; 
        ...
        /* Khung chứa nội dung chính giữa màn hình */
        .container { 
            max-width: 900px; 
            margin: 32px auto; 
            padding: 0 20px 40px; 
        ...
        /* Nút quay lại danh sách bài thi */
        .back-link { 
            display: inline-flex; 
            align-items: center; 
            gap: 8px; 
        ...
        /* Khung hiển thị chi tiết của một câu hỏi */
        .answer-card { 
            background: white; 
            padding: 24px; 
            margin-bottom: 20px; 
        ...
        /* Tô màu đáp án học sinh đã chọn (Màu xanh dương) */
        .option.user-ans { 
            border-color: #0277bd; 
            background: #dbeafe; 
        }
        
        /* Tô màu đáp án đúng (Màu xanh lá) */
        .option.correct-ans { 
            border-color: #16a34a; 
            background: #dcfce7; 
        }
        
        /* Tô màu đỏ nếu học sinh chọn sai */
        .option.wrong-ans { 
            border-color: #dc2626; 
            background: #fee2e2; 
        }
        
        /* Dòng hiển thị kết quả đúng/sai ở cuối mỗi câu */
        .result-row { 
            display: flex; 
            gap: 12px; 
            padding-top: 12px; 
        ...
        /* Ẩn bỏ nút biểu tượng (icon) theo yêu cầu */
