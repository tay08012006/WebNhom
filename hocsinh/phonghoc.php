<?php
session_start();
include '../dp.php'; // Kết nối CSDL từ file gốc
...
$sql_check = "SELECT id FROM hocsinh_lop WHERE id_hocsinh = ? AND id_lop = ?";
...
$sql_lop = "SELECT lop_hoc.*, users.ho_ten AS ten_gv 
            FROM lop_hoc 
...
// Trả về thông tin bài tập kèm theo ngày nộp nếu đã nộp
$sql_baitap = "SELECT b.*, n.ngay_nop 
               FROM baitap b 
               LEFT JOIN nop_bai_tap n ON b.id = n.id_baitap AND n.id_hocsinh = ?
               WHERE b.id_lop = ? 
...
$current_page = 'lophoc.php'; // Giữ active menu "Lớp học của tôi" bên Sidebar
?>
<!DOCTYPE html>
<html lang="vi">
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Nunito', sans-serif; }
        ...
        /* DANH SÁCH BÀI TẬP (Khung lớn bọc ngoài) */
        ...
        .deadline-highlight { color: #e53935; } /* Chữ màu đỏ cho hạn chót */

        /* Nút trạng thái nộp bài */
        ...
        /* 3 trạng thái màu nút theo chuẩn thiết kế của bạn */
        .status-yet { background: #fff3e0; color: #ef6c00; } /* Nộp bài ngay */
        .status-yet:hover { background: #ffe0b2; }
        
        .status-done { background: #e8f5e9; color: #2e7d32; pointer-events: none; } /* Đã nộp bài */
        
        .status-expired { background: #ffebee; color: #c62828; pointer-events: none; } /* Quá hạn nộp */
    </style>
</head>
<body>
        ...
        <div class="assignment-container">
            ...
            if ($result_baitap->num_rows > 0):
                while($row = $result_baitap->fetch_assoc()):
                    ...
                    // Định dạng hiển thị ngày giờ hạn nộp
                    $formatted_deadline = date("H:i d/m/Y", $han_nop_timestamp);
            ?>
                <div class="assignment-row">
                    <div class="assignment-info">
                        <h3><?php echo htmlspecialchars($row['ten_bai_tap']); ?></h3>
                    ...
                    <div class="assignment-action">
                        <?php 
                        // KIỂM TRA ĐỂ ĐỔ TRẠNG THÁI NÚT CHÍNH XÁC
                        if (!empty($row['ngay_nop'])) {
                            // 1. Nếu đã có dữ liệu trong cột ngay_nop tức là đã làm bài
                            echo '<span class="btn-status status-done">Đã nộp bài</span>';
                        } elseif ($current_timestamp > $han_nop_timestamp) {
                            // 2. Nếu chưa nộp mà thời gian hiện tại lớn hơn hạn nộp => Quá hạn
                            echo '<span class="btn-status status-expired">Quá hạn nộp</span>';
                        } else {
                            // 3. Còn hạn và chưa nộp => Cho phép bấm nộp bài
            ...
            ?>
                <p style="color:#78909c; font-weight:600; padding: 40px; text-align: center;">
                    Tuyệt vời! Hiện tại lớp học này chưa có bài tập nào cần làm.
                </p>
            <?php endif; ?>
        </div>
    </div>
