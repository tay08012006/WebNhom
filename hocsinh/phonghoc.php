...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Nunito', sans-serif; }
        ...
        /* === BÌNH LUẬN === */
        .bl-section { margin-top: 16px; padding-top: 14px; border-top: 1px solid #e2e8f0; }
        .bl-item { display: flex; gap: 10px; margin-bottom: 10px; align-items: flex-start; }
        .bl-avatar { width: 32px; height: 32px; border-radius: 50%; background: #e1f5fe; display: flex; align-items: center; justify-content: center; font-weight: 800; font-size: 12px; color: #0277bd; flex-shrink: 0; overflow: hidden; }
        .bl-avatar img { width: 100%; height: 100%; object-fit: cover; }
        .bl-bubble { background: #f8fafc; border-radius: 12px; padding: 9px 13px; flex: 1; }
        .bl-name { font-size: 13px; font-weight: 800; color: #1e293b; margin-bottom: 3px; }
        .bl-text { font-size: 14px; color: #374151; line-height: 1.5; }
        .bl-time { font-size: 11px; color: #94a3b8; margin-top: 3px; }
        .bl-form { display: flex; gap: 8px; margin-top: 12px; align-items: center; }
        .bl-input { flex: 1; padding: 9px 14px; border: 1px solid #cbd5e1; border-radius: 20px; font-family: inherit; font-size: 14px; outline: none; transition: border-color 0.2s; }
        .bl-input:focus { border-color: #1e3a8a; }
        .bl-send { width: 36px; height: 36px; border-radius: 50%; background: #1e3a8a; border: none; color: white; display: flex; align-items: center; justify-content: center; cursor: pointer; flex-shrink: 0; transition: 0.2s; }
        .bl-send:hover { background: #1d4ed8; }
    </style>
</head>
<body>
                            ...
                            <?php if (!empty($post['file_dinh_kem'])): ?>
                                <div style="margin:12px 0;padding:10px 14px;background:#f0f9ff;border:1px solid #bae6fd;border-radius:8px;">
                                    ...
                                    <?php foreach(array_filter(explode(',', $post['file_dinh_kem'])) as $fn): $fn=trim($fn); ?>
                                        <a href="../uploads/<?= htmlspecialchars($fn) ?>"
                                           target="_blank"
                            ...
                            <div class="bl-section">
                                    ...
                                    <div class="bl-avatar">
                                        <?php if ($bl_av): ?>
                                            <img src="<?= htmlspecialchars($bl_av) ?>" 
                                                 onerror="this.parentNode.textContent='<?= $bl_char ?>'">
                                        <?php else: ?>
                                            <?= htmlspecialchars($bl_char) ?>
                                        <?php endif; ?>
                                    </div>
                                    <div class="bl-bubble" <?= $is_gv ? 'style="background:#eff6ff;border-left:3px solid #1e3a8a;"' : '' ?>>
                                        <div class="bl-name">
                                            <?= htmlspecialchars($bl['hoten']) ?>
                                            <?php if ($is_gv): ?>
                                                <span style="font-size:11px;background:#1e3a8a;color:white;padding:2px 7px;border-radius:10px;margin-left:4px;font-weight:700;">GV</span>
                                            <?php endif; ?>
                                        </div>
                                ...
                                <form action="xuly_binhluan.php" method="POST" class="bl-form">
                                    ...
                                    <input type="text" name="noi_dung_bl" class="bl-input" 
                                           placeholder="Viết bình luận công khai tại đây...">
                                    <button type="submit" class="bl-send" title="Gửi">
                            ...
                            <div style="margin-bottom: 15px;">
                                ...
                                <?php if (!empty($btap['file_dinh_kem'])): ?>
                                    <div style="margin-top:12px;padding:10px 14px;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;">
                                        <p style="font-size:13px;font-weight:700;color:#15803d;margin-bottom:8px;">📎 Tài liệu đính kèm từ giáo viên:</p>
                                        <?php foreach(array_filter(explode(',', $btap['file_dinh_kem'])) as $fn): $fn=trim($fn); ?>
                                            <a href="../uploads/<?= htmlspecialchars($fn) ?>"
                                               target="_blank"
                                               style="display:inline-flex;align-items:center;gap:6px;background:white;border:1px solid #bbf7d0;padding:6px 12px;border-radius:6px;font-size:13px;font-weight:600;color:#15803d;text-decoration:none;margin:3px;">
                                                📄 <?= htmlspecialchars($fn) ?>
                                            </a>
                                        <?php endforeach; ?>
                                    </div>
