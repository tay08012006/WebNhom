...
// Lấy tên file hiện tại để kích hoạt trạng thái thẻ đang được chọn
$current_page = basename($_SERVER['PHP_SELF']);

// Kiểm tra Session họ tên để lấy chữ cái đầu làm avatar mặc định
...
// Đọc avatar thực tế từ database (nếu có)
$hs_avatar_src = '';
...
<script>
    // Kiểm tra ngay lập tức chế độ Dark Mode từ LocalStorage trước khi render giao diện
    (function() {
...
<style>
    /* BỘ BIẾN MÀU SẮC GIAO DIỆN SÁNG (MẶC ĐỊNH) */
    :root {
    ...
    /* BỘ BIẾN MÀU SẮC GIAO DIỆN TỐI (DARK MODE) */
    html.dark-mode {
    ...
    /* Áp dụng màu nền Dark Mode lên toàn body */
    html.dark-mode body { 
        background-color: #0f172a; 
        color: var(--text-chinh); 
    }

    /* 1. THANH HEADER NGANG BÊN TRÊN (NAVBAR) */
    .header-ngang {
        position: fixed; 
        top: 0; 
        left: 0; 
        ...
        border-bottom: 1px solid var(--border-mau); 
        z-index: 99;
        display: flex; 
        align-items: center; 
    ...
    /* Nút Tham gia lớp học trên Header */
    .btn-tham-gia-header {
    ...
    /* 2. NÚT BA DẤU GẠCH ĐỂ ẨN/HIỆN SIDEBAR */
    .nut-ba-gạch {
        position: fixed; 
        top: 10px; 
        left: 20px; 
    ...
    /* 3. KHU VỰC AVATAR VÀ MENU DROPDOWN */
    .vung-tai-khoan { 
        position: relative; 
        display: flex; 
        align-items: center; 
    ...
    /* Vòng tròn hiển thị chữ cái đầu hoặc ảnh avatar */
    .avatar-tron {
        width: 38px; 
        height: 38px; 
        background: #0284c7; 
        ...
        font-weight: 700; 
        font-size: 16px; 
        border: 2px solid var(--border-mau);
        text-transform: uppercase; 
        user-select: none;
    }
    
    /* Menu rớt xuống chứa nút Đăng xuất */
    .dropdown-thong-tin {
        position: absolute; 
        top: 48px; 
        right: 0; 
        ...
        border: 1px solid var(--border-mau);
        border-radius: 12px; 
    ...
    /* Các mục bên trong Menu Dropdown */
    .dropdown-item {
        display: flex; 
        align-items: center; 
    ...
    .dropdown-item.text-do { 
        color: #ef4444; 
    }
    .dropdown-item.text-do:hover { 
        background: #fff5f5; 
    }

    /* 4. THANH SIDEBAR MENU CHÍNH BÊN TRÁI */
    .thanh { 
        width: 260px; 
        background: var(--bg-sidebar); 
        padding: 80px 15px 30px 15px; 
        border-right: 1px solid var(--border-mau); 
        display: flex; 
        flex-direction: column; 
        position: fixed; 
        top: 0; 
    ...
    /* Trạng thái thanh Sidebar bị ẩn (Thụt vào trong) */
    .thanh.thut-vao { 
        transform: translateX(-260px); 
    }
    
    /* Tiêu đề Logo "Góc Học Tập" */
    .thanh h2 { 
        color: #0284c7; 
        font-weight: 800; 
        font-size: 20px; 
    ...
    /* Thẻ tùy chọn trên Sidebar (Trang chủ, Lớp học...) */
    .menu-item { 
    ...
    /* Đường kẻ mờ ngang phân cách trong Sidebar */
    .duong-ke-menu { 
        border-top: 1px solid var(--border-mau); 
        margin: 12px 10px; 
    }

    /* ==================== 5. GIAO DIỆN MODAL CÀI ĐẶT ==================== */
    /* Lớp mờ nền đằng sau Modal */
    .modal-overlay {
        position: fixed; 
        top: 0; 
        left: 0; 
    ...
    .modal-cai-dat {
        ...
        display: flex; 
        flex-direction: column;
        animation: moModal 0.25s ease-out; 
        color: var(--text-chinh);
    }
    
    /* Hiệu ứng mở Modal mượt mà */
    @keyframes moModal { 
        from { transform: scale(0.95); opacity: 0; } 
        to { transform: scale(1); opacity: 1; } 
    }
    
    /* Phần đầu của Modal Cài đặt */
    .modal-header { 
        padding: 18px 24px; 
        border-bottom: 1px solid var(--border-mau); 
        display: flex; 
    ...
    /* Phần thân chứa các mục thiết lập */
    .modal-body { 
        padding: 24px; 
        overflow-y: auto; 
        display: flex; 
    ...
    /* Từng dòng cấu hình riêng biệt */
    .setting-row { 
        display: flex; 
        justify-content: space-between; 
        align-items: center; 
        padding: 12px 0; 
        border-bottom: 1px dashed var(--border-mau); 
    }
    ...
    /* Màu nút gạt khi ở chế độ Dark Mode */
    html.dark-mode .slider-tron { 
        background-color: #475569; 
    }
    
    /* Màu nút gạt khi ở trạng thái bật (Checked) */
    input:checked + .slider-tron { 
        background-color: #2563eb; 
    }
    input:checked + .slider-tron:before { 
...
<script>
// Logic Đóng/Mở thanh Sidebar khi bấm vào biểu tượng 3 dấu gạch
...
// Logic Đóng/Mở Dropdown (Đăng xuất) khi click vào Avatar
...
// Đồng bộ trạng thái Dark Mode và các nút thông báo với LocalStorage khi tải trang
