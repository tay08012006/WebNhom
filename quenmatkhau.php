...
//  QUENMATKHAU.PHP — Quên mật khẩu (3 bước)
//  Bước 1: Xác minh email + môn học
//  Bước 2: Xác minh mã OTP gửi qua email
...
//  Hàm ẩn một phần địa chỉ email (vd: pha***@gmail.com)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'verify_identity') {
        ...
        if ($result->num_rows === 0) {
            ...
            // Tạo OTP 6 chữ số, hiệu lực 5 phút
            $otp = str_pad(random_int(0, 999999), 6, '0', STR_PAD_LEFT);

            // Xoá OTP cũ (nếu có) rồi lưu mới vào DB
    ...
    elseif ($action === 'verify_otp') {
        ...
        if (isset($_POST['resend'])) {
            ...
            if (strlen($entered) !== 6 || !ctype_digit($entered)) {
                ...
                // OTP đúng → đánh dấu đã dùng, chuyển sang bước đổi mật khẩu
                $user_id = (int)$_SESSION['reset_user_id'];
    ...
    elseif ($action === 'reset_password') {
        ...
        if (strlen($new_pass) < 6) {
            ...
            if ($upd->execute()) {
                // Xoá toàn bộ dữ liệu reset trong session
...
// Đồng bộ step từ session khi tải trang (GET)
...
(function() {
    ...
    // Rung và xoá ô khi nhập sai OTP
...
// Đếm ngược chuyển trang sau khi đổi mật khẩu thành công (bước 4)
(function() {
