...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['monhoc_yeuthich'])) {
        ...
        $stmt_check = $conn->prepare("SELECT * FROM users WHERE email = ? AND monhoc_yeuthich = ?");
        $stmt_check->bind_param("ss", $email, $monhoc);
        $stmt_check->execute();
    ...
    elseif (isset($_POST['new_password'])) {
        ...
        if ($new_pass !== $confirm_pass) {
            ...
            $stmt_update = $conn->prepare("UPDATE users SET matkhau = ? WHERE email = ?");
            $stmt_update->bind_param("ss", $hashed_pass, $email_reset);
